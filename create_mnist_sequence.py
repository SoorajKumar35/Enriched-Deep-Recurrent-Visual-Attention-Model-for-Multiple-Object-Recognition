...
def create_sample(x, output_shp, num_distortions=NUM_DISTORTIONS):
    ...
    for i,digit in enumerate(x):
        ...
        A = [28.0 / OUT_SHP[0], 0.0, y_start / float(OUT_SHP[0]), 0.0, 28.0 / OUT_SHP[1], x_start / float(OUT_SHP[1])]
        #print(A)
...
def create_dataset(n, X, labels, org_shp, out_shp):
    ...
    for i in range(n):
        ...
        digits, A = create_sample(x_, out_shp)
    ...
    return out_X.astype('float32'), np.vstack(out_lab).astype('float32')
...
samples = [create_sample(sample_digits(
    ...
    mnist_data['y_train'],
    ORG_SHP)[0], OUT_SHP)[0].reshape(-1) for i in range(400)]
...
np.save(outfile+"/X_train.npy", X_train)
np.save(outfile+"/X_test.npy", X_test)
np.save(outfile+"/X_valid.npy", X_valid)
np.save(outfile+"/y_train.npy", y_train)
np.save(outfile+"/y_test.npy", y_test)
np.save(outfile+"/y_valid.npy", y_valid)
"""
np.savez_compressed(
    outfile,
    X_train=X_train,
